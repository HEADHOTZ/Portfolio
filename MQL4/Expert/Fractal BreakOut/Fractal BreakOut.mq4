...
void OnTick()
{
   ...
   if(upperFractal!=0) { 
      ...
      if(date != iTime(Symbol(),0,0) && oldlowerFractal!=0) {
         ...
         ticket1 = OrderSend(_Symbol,OP_BUYSTOP,lotBuy,upperFractal,3,oldlowerFractal,(upperFractal+(profit*_Point)),NULL,0,0,clrGreen);
   ...
   if(date != iTime(Symbol(),0,0) && oldupperFractal!=0) {
      ...
      ticket2 = OrderSend(_Symbol,OP_SELLSTOP,lotSell,lowerFractal,3,oldupperFractal,(lowerFractal-(profit*_Point)),NULL,0,0,clrRed);
      date = iTime(Symbol(),0,0);
   }
   }
